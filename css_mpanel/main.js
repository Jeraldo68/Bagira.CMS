...
jQuery(document).ready(function() {
	...
	var dropZone = $('#dropZone'),
		maxFileSize = 8000000; // максимальный размер файла - 8 мб.
	...
	if (dropZone.length == 1) {
		var ext_arr = ['png', 'jpg', 'jpeg', 'bmp', 'rar', 'tar', 'zip', 'gzip', 'pdf', 'mp3', 'doc', 'docx'];
		...
		dropZone[0].ondrop = function(event) {
			...
			for (var i = 0, length = file.length; i < length; i++) {
				if (file[i].size > maxFileSize) {
					dropZone.text('Один или несколько файлов слишком большие!');
					dropZone.addClass('error');
					return false;
				}
				...
				if (!ext_arr.in_array(ext)) {
					dropZone.text('Данный тип файлов запрещен!');
					dropZone.addClass('error');
					return false;
				}
		...
		if (event.target.readyState == 4) {
			if (event.target.status == 200) {
				...
				if ($('#addto[checked]').length > 0)
					dropZone.nextAll('.dropZoneList').prepend(event.target.responseText);
				else
					dropZone.nextAll('.dropZoneList').append(event.target.responseText);
	...
	//удаление объектов из uploader'a
	$(".dropZoneList").on('click', '.dropListDelBtn', function() {

		var _this = $(this),
			url = _this.attr("href"),
			name = _this.parents('.dropListItem:first').find('a:first').html();

		ShowMessage(
			'Удаление объекта '+name,
			'Вы точно хотите удалить объект '+name+'?',
			{
				'Отмена': function() {
					$(this).dialog('close');
				},
				'Удалить': function() {
					$.get(url, function(data){
						if (data == 'delete') {
							_this.parents('.dropListItem:first').remove();
						}
					});
					$(this).dialog('close');
				}
