...
class ormEditForm {
	...
	*/
	public function getHTML($templ_name = 'orm_edit_form') {
		...
		if (file_exists($file)){
            ...
            if (system::$isAdmin && $this->obj->getClass()->isPage() && !$this->without_sh) {
	...
	*/
	...
	private function parseField($field, $TEMPLATE) {
		...
		if ($this->obj->getClass()->isPage() && in_array($field['f_sname'], $this->page_fields)) {
        ...
        if ($field['f_type'] == 20)
        	$this->java .= page::parse('rules.push("reg_exp,%field.sname%,^(((f|ht){1}tp(s)?:/)*/[-a-zA-Z0-9@:%_\+.~#?&//=]+)*$|^\/{1}, '.lang::get('CONSTR_BASE_FIELD_E4').'");');
